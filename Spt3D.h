...
// =========================================================================
//  IMMEDIATE-MODE DRAWING（不经过 Pipeline，简单场景 / debug 用）
// =========================================================================

void BeginMode3D(const Camera3D& cam);
void EndMode3D();
void BeginMode2D(const Camera2D& cam);
void EndMode2D();
void BeginRT(RenderTarget rt);
void EndRT();
void BeginBlend(BlendMode m);
void EndBlend();
void BeginScissor(int x, int y, int w, int h);
void EndScissor();
void Flush();

// immediate 3D drawing
void DrawMesh(Mesh mesh, Material mat, Mat4 transform);
void DrawMeshInstanced(Mesh mesh, Material mat, const Mat4* xforms, int n);
void DrawModel(Model model, Vec3 pos, float scale, Color tint = Colors::White);
void DrawModel(Model model, Transform t, Color tint = Colors::White);
void DrawBillboard(const Camera3D& cam, Texture tex, Vec3 pos, float size, Color tint = Colors::White);

// immediate 3D debug
void DbgLine(Vec3 a, Vec3 b, Color c);
void DbgCube(Vec3 pos, Vec3 size, Color c);
void DbgSphere(Vec3 center, float r, Color c);
void DbgCapsule(Vec3 start, Vec3 end, float r, Color c);
...
void DrawTex(Texture tex, Vec2 pos, Color tint = Colors::White);
void DrawTex(Texture tex, Rect src, Vec2 pos, Color tint = Colors::White);
void DrawTex(Texture tex, Rect src, Rect dst, Color tint = Colors::White);
void DrawSprite(Texture tex, Vec2 pos, Vec2 origin, float rot, Vec2 scale, Color tint = Colors::White);
void DrawSpritePro(Texture tex, Rect src, Rect dst, Vec2 origin, float rot, Color tint = Colors::White);
void DrawNinePatch(Texture tex, Rect src, Rect dst, Rect border, Color tint = Colors::White);

void DrawPixel(Vec2 pos, Color c);
void DrawLine(Vec2 a, Vec2 b, float thick, Color c);
void DrawLineStrip(const Vec2* pts, int n, float thick, Color c);
void DrawBezier(Vec2 a, Vec2 b, float thick, Color c);
void DrawRect(Rect rect, Color c);
void DrawRectPro(Rect rect, Vec2 origin, float rot, Color c);
void DrawRectGrad(Rect rect, Color tl, Color tr, Color bl, Color br);
void DrawRectLines(Rect rect, float thick, Color c);
void DrawRectRounded(Rect rect, float round, int seg, Color c);
void DrawRectRoundedLines(Rect rect, float round, int seg, float thick, Color c);
void DrawCircle(Vec2 center, float r, Color c);
void DrawCircleGrad(Vec2 center, float r, Color inner, Color outer);
void DrawCircleLines(Vec2 center, float r, float thick, Color c);
void DrawCircleSector(Vec2 center, float r, float start, float end, int seg, Color c);
void DrawEllipse(Vec2 center, float rx, float ry, Color c);
void DrawEllipseLines(Vec2 center, float rx, float ry, float thick, Color c);
void DrawRing(Vec2 center, float inner, float outer, float start, float end, int seg, Color c);
void DrawTriangle(Vec2 a, Vec2 b, Vec2 c, Color color);
void DrawTriangleLines(Vec2 a, Vec2 b, Vec2 c, float thick, Color color);
void DrawTriangleFan(const Vec2* pts, int n, Color c);
void DrawPoly(Vec2 center, int sides, float r, float rot, Color c);
void DrawPolyLines(Vec2 center, int sides, float r, float rot, float thick, Color c);
