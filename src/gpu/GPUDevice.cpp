...
namespace spt3d {
    ...
    static GLuint CompileShader(GLenum type, const char* src) {
        ...
        if (!success) {
            ...
            std::cerr << "[GPU] Shader compilation failed: " << log << std::endl;
            std::cerr << "[GPU] Shader source: " << src << std::endl;
    ...
    static GLuint LinkProgram(GLuint vs, GLuint fs) {
        ...
        if (!success) {
            ...
            std::cerr << "[GPU] Program link failed: " << log << std::endl;
        ...
        if (linked) {
            std::cout << "[GPU] Program linked successfully: " << program << std::endl;
        }
    ...
    ShaderHandle GPUDevice::createShader(const ShaderDesc& desc) {
        ...
        for (const auto& passDesc : desc.passes) {
            ...
            for (int i = 0; i < numUniforms; ++i) {
                ...
                if (loc >= 0) {
                    // Store with the raw driver-reported name
                    pass.uniformLocs[name] = loc;
                    int32_t nameHash = HashName(std::string_view(name));
                    pass.uniformHashLocs[nameHash] = loc;

                    // Some Android GL drivers append "[0]" to non-array
                    // uniforms in glGetActiveUniform. Also store the
                    // stripped base name so lookups by "u_model" still
                    // match when the driver reported "u_model[0]".
                    std::string rawName(name);
                    auto bracket = rawName.find('[');
                    if (bracket != std::string::npos) {
                        std::string baseName = rawName.substr(0, bracket);
                        pass.uniformLocs[baseName] = loc;
                        int32_t baseHash = HashName(std::string_view(baseName));
                        pass.uniformHashLocs[baseHash] = loc;
                    }
                }
            }
    ...
    void GPUDevice::bindMesh(MeshHandle h) {
        ...
        MeshResource* res = m_impl->meshes.get(h.value);
        if (!res) {
            std::cerr << "[GPU] bindMesh: mesh not found for handle " << h.value << std::endl;
            return;
        }
        ...
        // 部分安卓 GL 驱动（微信小游戏 WebView 等）在切换 VAO 后
        // 不能正确恢复 GL_ELEMENT_ARRAY_BUFFER 绑定状态。
        // 显式重新绑定 IBO 确保索引绘制正常工作。
        // 在合规驱动上这是无害的冗余操作。
        if (res->indexed && res->ibo) {
            glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, res->ibo);
        }
    }
    ...
    void GPUDevice::useShader(ShaderHandle h, std::string_view passName) {
        if (!isValid(h)) {
            std::cerr << "[GPU] useShader: invalid handle" << std::endl;
        ...
        ShaderResource* res = m_impl->shaders.get(h.value);
        if (!res) {
            std::cerr << "[GPU] useShader: shader not found for handle " << h.value << std::endl;
            return;
        }
    ...
    static GLint GetUniformLocation(ShaderResource* res, std::string_view name) {
        ...
        for (auto& pass : res->passes) {
            ...
            // Fallback: query GL directly (handles any driver name quirk)
            GLint loc = glGetUniformLocation(pass.program, key.c_str());
            if (loc >= 0) {
                pass.uniformLocs[key] = loc;   // cache for next time
                return loc;
            }
        }
        return -1;
    ...
    void GPUDevice::drawIndexed(int indexCount) {
        if (m_impl->boundVAO == 0) {
            std::cerr << "[GPU] drawIndexed failed: boundVAO is 0" << std::endl;
            return;
        }
        ...
        if (!mesh) {
            std::cerr << "[GPU] drawIndexed failed: mesh not found for VAO " << m_impl->boundVAO << std::endl;
            return;
        }
        if (!mesh->indexed) {
            std::cerr << "[GPU] drawIndexed failed: mesh is not indexed" << std::endl;
            return;
        }
    ...
    void GPUDevice::drawArrays(int vertexCount) {
        if (m_impl->boundVAO == 0) {
            std::cerr << "[GPU] drawArrays failed: boundVAO is 0" << std::endl;
            return;
        }
        ...
        if (!mesh) {
            std::cerr << "[GPU] drawArrays failed: mesh not found for VAO " << m_impl->boundVAO << std::endl;
            return;
        }
    ...
    void GPUDevice::drawIndexedInstanced(int indexCount, int instanceCount) {
        ...
        glDrawElementsInstanced(GL_TRIANGLES,
                                indexCount > 0 ? indexCount : mesh->indexCount,
                                idxType, nullptr,
                                instanceCount);
    }

    void GPUDevice::drawArraysInstanced(int vertexCount, int instanceCount) {
        if (m_impl->boundVAO == 0) return;
        ...
        glDrawArraysInstanced(GL_TRIANGLES, 0,
                              vertexCount > 0 ? vertexCount : mesh->vertexCount,
                              instanceCount);
    }

    void GPUDevice::setInstanceBuffer(MeshHandle mesh, const void* data, size_t size, size_t stride) {
        if (!isValid(mesh)) return;
        MeshResource* res = m_impl->meshes.get(mesh.value);
        if (!res) return;
        ...
        glVertexAttribPointer(location, 4, GL_FLOAT, GL_FALSE, static_cast<GLsizei>(stride), nullptr);
        glVertexAttribDivisor(location, 1);

        if (stride >= 32) {
            glEnableVertexAttribArray(location + 1);
            glVertexAttribPointer(location + 1, 4, GL_FLOAT, GL_FALSE, static_cast<GLsizei>(stride), reinterpret_cast<void*>(16));
            glVertexAttribDivisor(location + 1, 1);
        }
        if (stride >= 48) {
            glEnableVertexAttribArray(location + 2);
            glVertexAttribPointer(location + 2, 4, GL_FLOAT, GL_FALSE, static_cast<GLsizei>(stride), reinterpret_cast<void*>(32));
            glVertexAttribDivisor(location + 2, 1);
        }
        if (stride >= 64) {
            glEnableVertexAttribArray(location + 3);
            glVertexAttribPointer(location + 3, 4, GL_FLOAT, GL_FALSE, static_cast<GLsizei>(stride), reinterpret_cast<void*>(48));
    ...
    void GPUDevice::drawBuffers(int count, const int* attachments) {
